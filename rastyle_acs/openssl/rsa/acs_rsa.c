...
char* rsa_key_seliaze(char* key_data)
{
	int i;
	char *rsa_key = NULL;
    int offset = 0;
...
//	int nPublicKeyLen = strPublicKey.size();      //strPublicKey为base64编码的公钥字符串
	printf("key len is %d \n",key_data_len);
	rsa_key = (char *)malloc(key_data_len+100);
	if(rsa_key)
	{
		memset(rsa_key,0,key_data_len+100);
	}
	else
	{
		return NULL;
	}
	strcat(rsa_key,"-----BEGIN RSA PUBLIC KEY-----\n");
	offset += strlen("-----BEGIN RSA PUBLIC KEY-----\n");
	for(i = 64; i < key_data_len; i+=64)
	{
		if(key_data[i] != '\n')
		{
			memcpy(rsa_key+offset,key_data+i-64,64);
			offset += 64;
			strcat(rsa_key+offset,"\n");
			offset +=1;
		}
		i++;
	}
	printf("offset is %d\n",offset);
	printf("len is %d,str is %s \n",strlen(key_data+i-64),key_data+i-64);
	memcpy(rsa_key+offset,key_data+i-64,strlen(key_data+i-64));
	offset += strlen(key_data+i-64);
	printf("offset is %d \n",offset);
	strcat(rsa_key+offset,"\n-----END RSA PUBLIC KEY-----\n");
	return rsa_key;
...
char *js_private_decrypt(const char *cipher_text,int length)
{
...
char *js_public_encrypt(const char *plain_text,int * length)
{
